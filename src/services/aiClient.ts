...
const agnesChatCompletionsUrl = "https://apihub.agnes-ai.com/v1/chat/completions";
...
const validAnswers = new Set<AiAnswerText>(["是", "不是", "是也不是", "无关"]);
const labelByAnswer = {
  是: "yes",
  不是: "no",
  是也不是: "both",
...
async function askProxy(apiUrl: string, request: AiRequest): Promise<AiResponse> {
  ...
  if (!response.ok) {
    throw new AiProxyError(`AI API failed with ${response.status}`, response.status);
  }
...
async function askAgnesDirect(request: AiRequest): Promise<AiResponse> {
  ...
  return parseModelOutput(data.choices?.[0]?.message?.content ?? "", request.hintEnabled);
}
...
function buildMessages(request: AiRequest) {
  ...
  const user = [`汤面：${request.surface}`, `汤底：${request.truth}`, `问题：${request.question}`].join("\n\n");
...
function parseModelOutput(content: string, hintEnabled: boolean): AiResponse {
  ...
  try {
    const parsed = JSON.parse(jsonText) as Partial<{ answer: string; hint: string }>;
    return normalizeAiResponse(
      {
        answer: normalizeAnswer(parsed.answer || fallbackAnswer),
        hint: typeof parsed.hint === "string" ? parsed.hint.trim().slice(0, 80) : undefined,
      },
      hintEnabled,
    );
  } catch {
...
function normalizeAnswer(answer: string): AiAnswerText {
  return validAnswers.has(answer as AiAnswerText) ? (answer as AiAnswerText) : "无关";
}

function normalizeAiResponse(data: Partial<AiResponse>, hintEnabled: boolean): AiResponse {
  const answer = data.answer && validAnswers.has(data.answer) ? data.answer : "无关";
