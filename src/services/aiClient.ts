...
import { ApiKeyUnlockError, getApiKeyForModel } from './apiKeyVault'

const fallbackApiBaseUrls: Record<AiModelId, string> = {
...
export async function askAi(request: AiRequest): Promise<AiResponse> {
  ...
  const apiUrl = `${getApiBaseUrl(request.model)}/chat/completions`
  const prompt = buildPrompt(request)
  const body = JSON.stringify({
    model: request.model,
    temperature: 0.1,
    max_tokens: prompt.maxTokens,
    ...getExtraBody(request.model),
    messages: [
      {
        role: 'system',
  ...
  return askSource(apiUrl, apiKey, body, request)
}
...
async function askSource(
  ...
  return parseModelOutput(
    content,
    request.hintEnabled,
    normalizeHintCandidates(request.hintCandidates),
    request.question,
  )
}

function getApiBaseUrl(model: AiModelId) {
  const configuredApiBaseUrl = import.meta.env.VITE_AI_API_BASE_URL as
    | string
    | undefined
  const baseUrl = configuredApiBaseUrl?.trim() || fallbackApiBaseUrls[model]

  return trimTrailingSlash(baseUrl)
}

function getExtraBody(model: AiModelId) {
...
function parseModelOutput(
  content: string,
  hintEnabled: boolean,
  hintCandidates: Array<{ index: number; text: string }> = [],
  question = '',
): AiResponse {
...
function normalizeHintCandidates(value: unknown) {
  ...
  return value
    .filter(
      (candidate): candidate is { index: number; text: string } =>
        !!candidate &&
...
function normalizeAnswer(value: string): AiAnswerText {
  if (validAnswers.has(value as AiAnswerText)) {
    return value as AiAnswerText
  }
...
function extractAnswer(content: string): AiAnswerText {
  const answerMatch = content.match(/还原正确|是也不是|不是|无关|是/)
  return normalizeAnswer(answerMatch?.[0] ?? '无关')
}
...
class AiSourceError extends Error {
  constructor(
    message: string,
